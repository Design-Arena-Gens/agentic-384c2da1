...
## Forex Scanner Script

The repository includes `forex_scanner.py`, a Python script that retrieves live exchange rates for multiple currency pairs from the free Alpha Vantage API.
...
1. Install dependencies:
   ...
   ```
2. Export your Alpha Vantage API key (replace `<YOUR_KEY>` with the key you obtained from https://www.alphavantage.co):
   ```bash
...
- Fetch the default set of pairs and display a table:
  ```bash
  python3 forex_scanner.py
  ```

- Scan specific pairs:
  ```bash
  python3 forex_scanner.py --pairs EUR/USD USD/JPY AUD/USD
  ```

- Output JSON or save results:
  ```bash
  python3 forex_scanner.py --output json --save rates.json
  ```

- Run without an API key using bundled demo data:
  ```bash
  python3 forex_scanner.py --demo
  ```
